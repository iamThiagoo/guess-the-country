<template>
  ...
  >
    ...
    <template #content>
      <div
        class="w-80 p-3 rounded-lg bg-slate-800 text-white text-sm shadow-xl border border-violet-500 space-y-1"
      >
        <div v-html="currentTips" />
      </div>
    </template>
  </UPopover>
</template>
...
<script setup>
  ...
  const currentTips = computed(() => {
    return `<ul>${game.$state.currentCountry.tips.map(t => `<li>• ${t}</li>`).join('')}</ul>`
  })
</script>
